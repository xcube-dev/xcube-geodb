...
class GeoDBTest(unittest.TestCase):
    ...
    def test_query_by_bbox(self, m):
        ...
        url = "http://test:3000"
        path = "/rpc/get_by_bbox"
        m.get(url="http://test:3000/", text=json.dumps({'paths': ['/rpc/get_by_bbox', '/dataset']}))
    ...
    def test_real(self):
        api = GeoDB()
        pp = [{"name": "tt2", "type": "integer"}]
        api.create_dataset('tt', pp)
        #print(api.filter_by_bbox(dataset="land_use", minx=452750.0, miny=88909.549, maxx=464000.0, maxy=102486.299,
